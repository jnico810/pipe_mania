...
class Game {
  ...
  start(){
    ...
    window.setTimeout(this.start.animate.bind(this.start, this.currentSludgeSpace.exit), 5000 );
  }
  ...
  generateSpaces(){
    for (let i = 0; i < 7; i++ ){
      ...
      for (let j = 0; j < 10; j++ ){
        if (i === 3 && j === 3){
          this.start = new Space([i,j], [i,j], spaces, "start", this.handleClick.bind(this),this.fillNextPipe.bind(this));
          ...
          space = new Space([i,j], [i,j], spaces, "tile", this.handleClick.bind(this), this.fillNextPipe.bind(this));
          space.createCanvas();
          row.push(space);
        }
      }
  ...
  generateSideTiles(){
    for (let i = 1; i < 6 ; i++ ){
      ...
      let option = new Space([0,0], `${i}choice`, spaces, "option", null, this.fillNextPipe.bind(this));
      option.createCanvas();
  ...
  handleClick(space, e){
    ...
    const newOption = new Space([0,0],[row,col], null, "option", null, this.fillNextPipe.bind(this) );
